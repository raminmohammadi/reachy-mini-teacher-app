...
from reachy_mini_teacher_app.config import DEFAULT_PROFILES_DIRECTORY, config
    ...
    try:
        if instructions_file.exists():
            ...
            if instructions:
                ...
                # Inject session recap (legacy placeholder, kept for other profiles)
                recap_text = recap if recap else "اولین جلسه با این کاربر است — سابقه‌ای وجود ندارد."
                expanded_instructions = expanded_instructions.replace(
                    "{previous_session_recap}", recap_text
                )
