...
from reachy_mini_teacher_app.config import config  # noqa: F401
from reachy_mini_teacher_app.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from reachy_mini_teacher_app.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "reachy_mini_teacher_app.profiles"
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"reachy_mini_teacher_app.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
...
def reinitialize_tools() -> None:
    """Force-reload the tool registry (e.g. after a profile switch)."""
    global ALL_TOOLS, ALL_TOOL_SPECS, _TOOLS_INITIALIZED
    _TOOLS_INITIALIZED = False
    ALL_TOOLS = {}
    ALL_TOOL_SPECS = []
    _initialize_tools()
    logger.info("Tools reinitialized — %d tools loaded", len(ALL_TOOLS))
