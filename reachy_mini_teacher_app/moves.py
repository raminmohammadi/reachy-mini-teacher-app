...
class MovementManager:
    ...
    def __init__(
        ...
        # Disconnection detection: after this many consecutive set_target failures
        # (~3 s at 100 Hz) the robot is considered disconnected and the loop
        # drops to 1 Hz to avoid wasting CPU and flooding logs.
        self._consecutive_errors: int = 0
        self._robot_connected: bool = True
        self._disconnect_threshold: int = 300  # ~3 s at 100 Hz
        self._disconnected_retry_interval: float = 1.0  # s between retries when disconnected
    ...
    def mark_activity(self) -> None:
        """Mark the robot as active, resetting the inactivity timer.
    ...
    def _issue_control_command(self, head: NDArray[np.float32], antennas: Tuple[float, float], body_yaw: float) -> bool:
        """Send the fused pose to the robot with throttled error logging.

        Returns True on success, False on failure.  Tracks consecutive errors
        so the caller can switch to a low-frequency retry mode when the robot
        ...
        try:
            ...
            # Transition to disconnected state after sustained failures
            if self._robot_connected and self._consecutive_errors >= self._disconnect_threshold:
                self._robot_connected = False
                logger.warning(
                    "Robot appears disconnected after %d consecutive set_target failures. "
                    "Reducing control loop to %.0f Hz until connection is restored.",
                    self._consecutive_errors,
                    1.0 / self._disconnected_retry_interval,
                )
            return False
        else:
            if not self._robot_connected:
                logger.info(
                    "Robot connection restored after %d failures.",
                    self._consecutive_errors,
                )
    ...
    def working_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            # 7) Adaptive sleep: slow-retry when robot is disconnected to avoid
            #    burning CPU and flooding logs at 100 Hz.
            self._publish_shared_state()
            if not self._robot_connected:
                # 1 Hz retry — keep checking for restoration without hammering
                time.sleep(self._disconnected_retry_interval)
                continue
