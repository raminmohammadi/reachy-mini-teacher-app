...
class CameraWorker:
    """
    A high-frequency background worker that manages the robot's camera feed and 
    automated head tracking.

    WHAT IT DOES:
    - Polling: Continuously grabs frames from the Reachy Mini's camera at 25-30Hz.
    - Thread-Safety: Buffers the latest frame so other tools (like Vision AI) can 
      grab a picture without interrupting the camera hardware.
    - Face Tracking: Coordinates with a HeadTracker to calculate where the robot 
      should look.
    - Smoothing: Implements 'Hysteresis' and 'Interpolation'—if a person walks away, 
      the robot waits 2 seconds and then slowly glides its head back to a 
      neutral position rather than snapping back instantly.

    WHY IT IS NEEDED:
    Camera hardware is notoriously 'blocking' (it makes the code wait). By putting 
    this in a separate thread, the robot's 'brain' stays fast. Additionally, 
    direct face-to-motor mapping is often shaky; the interpolation logic here 
    gives the Reachy Mini a weighted, organic feel to its head movements.
    """
